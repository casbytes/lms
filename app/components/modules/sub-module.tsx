...
type SubModuleItemProps = {
  user: types.User;
  submodule: types.SubModuleProgress;
  index: number;
};

export function SubModule({ submodule, index, user }: SubModuleItemProps) {
  const isActive = user.subscribed;
  const locked = submodule.status === Status.LOCKED || !isActive;
  const inProgress = submodule.status === Status.IN_PROGRESS;
